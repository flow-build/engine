...
const { getParameter } = require("../../cockpit/utils/parametersClient");
...
class Blueprint {
  ...
  static async validate_environment_variable(spec) {
    ...
    for await (const [key, value] of Object.entries(spec.environment)) {
      const variable = await getParameter(value);
      if (!variable && !process.env[value] && key === value?.toLowerCase()) {
  ...
  static async parseSpec(blueprint_spec) {
    ...
    for await (const [key, value] of Object.entries(result_spec.environment)) {
      const variable = await getParameter(value);
      result_spec.environment[key] = variable?.Value || process.env[value];
    }
    return result_spec;
  }
